...
impl ToState for MultiState {
    fn to_state(&self) -> State {
        BTreeSet::from_iter(self.clone()).into_iter().collect::<Vec<String>>().join(",")
    }
}
    ...
    pub fn from_nfa(nfa: Nfa) -> Dfa {
        match nfa.is_dfa() {
            ...
            _ => {
                ...
                let states: States = multi_nfa.states
                    .into_iter()
                    .map(|state| state.to_state())
                    .collect();
                let alphabet: Alphabet = multi_nfa.alphabet;
                let state_transitions: StateTransitions = multi_nfa.state_transitions
                    .into_iter()
                ...
                let final_states: States = multi_nfa.final_states
                    .into_iter()
                    .map(|state| state.to_state())
                    .collect();
                Dfa {
...
impl Nfa {
    pub fn is_dfa(&self) -> bool {
        !self.state_transitions.values()
            .any(|state_transitions| state_transitions.iter()
...
impl MultiNfa {
    pub fn from_nfa(nfa: &Nfa) -> MultiNfa {
        let mut all_transitions = MultiNfaStateTransitions::new();

        // First, find all states reachable by epsilon closure from the start state.
        let mut epsilon_reachable_state = nfa.start_state.to_multi_state();
        MultiNfa::epsilon_closure(&mut epsilon_reachable_state,
                                  &nfa.start_state,
                                  &nfa.state_transitions);

        // Recursively find all states reachable by reading input from the start state.
        MultiNfa::aggregate_transitions(&mut all_transitions,
                                        &nfa.start_state.to_multi_state(),
                                        &nfa.state_transitions);

        let states: MultiStates = all_transitions.keys().cloned().collect();
        ...
        MultiNfa {
            ...
            start_state: epsilon_reachable_state,
            final_states: final_states,
        }
    }

    fn epsilon_closure(reachable_mstate: &mut MultiState,
                       initial_state: &State,
                       state_transitions: &NfaStateTransitions) {
        match state_transitions.get(initial_state) {
            Some(initial_state_transitions) => match initial_state_transitions.get(&EPSILON) {
                Some(initial_state_epsilon_transitions) =>
                    for epsilon_transition_state in initial_state_epsilon_transitions.iter() {
                        if reachable_mstate.insert(epsilon_transition_state.clone()) {
                            ...
                            MultiNfa::epsilon_closure(reachable_mstate,
    ...
    fn aggregate_transitions(all_transitions: &mut MultiNfaStateTransitions,
                             current_state: &MultiState,
                             state_transitions: &NfaStateTransitions) {
        // Check if there is at least one transition for the current state.
        if current_state.iter().any(|state| state_transitions.get(state).is_none()) {
            return;
        };
        ...
        for state in current_state {
            MultiNfa::epsilon_closure(&mut reachable_current_mstate,
                                      state,
                                      state_transitions);
        };
        ...
        for reachable_state in reachable_current_mstate.iter() {
            match state_transitions.get(reachable_state) {
                Some(reachable_state_transitions) => {
                    for (symbol, reachable_state_transitions) in reachable_state_transitions {
                        if symbol.ne(&EPSILON) {
                            let mut reachable_states_with_epsilon_closure =
                                reachable_state_transitions.clone().into_iter()
                                    .collect::<MultiState>();
        ...
        all_transitions.insert(reachable_current_mstate, aggregated_current_state_transitions
            .clone());

        let reachable_states =
            aggregated_current_state_transitions.values().cloned().collect::<MultiStates>();
