...
fn assert_dfa_state(
    ...
    for state in states {
        assert!(
            dfa.get_states().contains(state),
            "state <{}> not found in states <{:#?}>",
            state,
            dfa.get_states()
        );
    }
    ...
    for symbol in alphabet.chars() {
        assert!(
            dfa.get_alphabet().contains(&symbol),
    ...
    for state in final_states {
        assert!(
            dfa.get_final_states().contains(state),
            "state <{}> not found in final states <{:#?}>",
            state,
    ...
    for expected_transition in transitions.iter() {
        let transition = dfa
            ...
            .expect(
                format!(
                    "Transitions not found for state <{:#?}> in transitions <{:#?}>",
...
fn assert_dfa_acceptance(dfa: Dfa, languages: TestLanguages, acceptance: Acceptance) {
    for language in languages {
        assert_eq!(
            acceptance,
            dfa.accepts(&language.to_string()),
            "language <{}>",
            language
        );
    }
...
fn rejects() {
    ...
    assert_dfa_acceptance(dfa, vec!["00000", "01010", "001000"], Acceptance::Rejected);
}
...
fn deserialize_nfa() {
    ...
    assert_dfa_state(
        dfa,
        vec![
            "q1", "q2", "q3", "q0,q1", "q1,q2", "q1,q3", "q2,q3", "q0,q1,q2",
        ],
        "ab",
        "q0,q1",
        vec!["q0,q1", "q0,q1,q2"],
